...
func TestMain(m *testing.M) {
	if !setup() {
		// envtest binaries (etcd, kube-apiserver) not available; skip this suite.
		os.Exit(0)
	}
	defer teardown()
	code := m.Run()
...
// setup returns false if envtest cannot be started (e.g. missing etcd binaries).
func setup() bool {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Skipping api/v1beta1 integration tests (envtest not available): %v\n", err)
		return false
	}
